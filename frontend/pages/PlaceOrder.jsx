  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
        ...
        if (cartItems[itemId] > 0) {
          ...
          if (itemInfo) {
            orderItems.push({
              ...
              priceAtPurchase: itemInfo.price
            });
          }
        }
      });
      ...
      let orderData = {
        address,
        items: orderItems,
        // amount removed, backend will calculate
      ...
      switch (method) {
        ...
        case 'stripe':
        case 'razorpay':
          const payResponse = await axios.post(backendUrl + `/api/order/${method}`, orderData, { headers: { Authorization: `Bearer ${token}` } });
          if (payResponse.data.success) {    
            const {session_url} = payResponse.data;
            window.location.href = session_url; // Redirect to payment gateway
          } else {
